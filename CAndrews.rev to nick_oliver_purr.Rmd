...
## Additonal Plot on Document - Upated by Coffy Andrews-Guo
[RPubs]()
...
*Modification to Data Plot - Visualization based on Party Affiliations*
```{r}
# Default Plot
library(ggplot2)
new_plot <- approvalPollsDf %>% ggplot(aes(x=party,y=map_dbl(approve_dissapprove_diff,abs),fill=majority_approve)) +
  geom_boxplot() 

# Add titles 
new_plot <- new_plot + ggtitle("Approval and Disapproval by Party Affliations") + theme(plot.title.position = "plot", plot.title = element_text(hjust = 0.5))
new_plot
```
